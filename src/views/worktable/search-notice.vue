<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="搜索通知标题、通知内容" style="width: 320px;" class="filter-item" @keyup.enter.native="searchBlank" minlength="1" maxlength="32" show-word-limit/>
...
<script>
import { deleteByBookSuitId, fetchList,searchBook } from '@/api/notice'
  ...
  methods: {
    ...
    handleDelete(row, index) {
      this.$confirm('此操作将永久删除该通知, 是否继续?', '提示', {
        ...
        deleteByBookSuitId(row.noticeId).then(response => {
    ...
    handleTop(row, index) {
      topBook(row.noticeId).then(response => {
        console.log(response)
        this.$notify({
        title: '成功',
        message: response.msg,
    ...
    cancelTopBook(row, index) {
      ...
      cancelTopBook(row.noticeId).then(response => {
        console.log(response)
        this.$notify({
        title: '成功',
        message: response.msg,
